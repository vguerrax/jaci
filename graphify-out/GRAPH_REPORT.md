...
## Corpus Check
- 131 files · ~133,201 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1661 nodes · 4139 edges · 87 communities (82 shown, 5 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 247 edges (avg confidence: 0.73)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `52de4c3a`
...
1. `User` - 144 edges
2. `make_user()` - 91 edges
3. `make_group()` - 90 edges
4. `create_template()` - 74 edges
5. `create_execution_from_template()` - 73 edges
6. `Execution` - 65 edges
7. `ExecutionItem` - 50 edges
8. `add_item_to_template()` - 50 edges
9. `Group` - 44 edges
...
Cohesion: 0.05
Nodes (137): RecurrenceType, calculate_next_date(), generate_next_execution(), Calcula a próxima data baseada na recorrência. Usado para geração automática de…, Gera a próxima execução do ciclo, se aplicável. Condições: - Execução vinculada…, add_item_to_execution(), complete_item(), create_execution_from_pending() (+129 more)
...
Cohesion: 0.06
Nodes (77): Template, TemplateItem, create_template_page(), edit_template_page(), _get_template_context(), handle_add_item(), handle_create_template(), handle_delete_template() (+69 more)
...
Cohesion: 0.08
Nodes (50): agenda_calendar(), agenda_day(), agenda_list(), handle_reschedule(), get, post, Request, Session (+42 more)

### Community 6 - "User"
Cohesion: 0.06
Nodes (91): Dependência que exige usuário autenticado. Levanta exceção se não autenticado., require_user(), Group, Notification, User, close_execution_page(), complete_item_form(), _completed_page() (+83 more)
...
Cohesion: 0.11
Nodes (41): create_group_page(), group_detail(), handle_create_group(), handle_edit_group(), handle_invite(), handle_remove_member(), handle_switch_group(), list_groups() (+33 more)
...
### Community 24 - "utils/__init__.py"
Cohesion: 0.11
...
Cohesion: 0.18
Nodes (12): Base, ensure_schema_compatibility(), Compatibilidade temporária para bancos SQLite anteriores ao Alembic., set_sqlite_pragma(), Category, TemplateLearningDismissal, _iso(), datetime (+4 more)
...
Cohesion: 0.25
Nodes (15): create_user(), _credentials(), _jwks(), logout(), migrate_user(), Any, refresh(), _request() (+7 more)
...
### Community 65 - "format_local_date"
Cohesion: 0.28
...
### Community 72 - "FL-10 — Home Operacional"
Cohesion: 0.24
Nodes (7): FL-10 — Home Operacional, Fluxo Principal, Fluxos Principais do Usuário — Jaci, Objetivo, Objetivo, Princípios Gerais, Resultado Esperado
...
### Community 79 - "FL-08 — Consulta da Agenda e Histórico"
Cohesion: 0.33
...
### Community 81 - "FL-06 — Aprendizado do Template"
Cohesion: 0.33
...
### Community 83 - "FL-05 — Compra Colaborativa"
Cohesion: 0.40
...
## Knowledge Gaps
- **421 isolated node(s):** `ACTIVE_CACHES`, `publish.sh script`, `backup_database.sh script`, `Projeto: Jaci`, `Princípios do Produto` (+416 more)
...
- **Why does `User` connect `User` to `make_user`, `template_service.py`, `Execution`, `auth.py`, `agenda_service.py`, `categories.py`, `groups.py`, `main.py`, `auth_service.py`, `test_pwa.py`, `tupa_auth_service.py`, `sync_center`, `datetime.py`?**
  _High betweenness centrality (0.098) - this node is a cross-community bridge._
- **Why does `Execution` connect `Execution` to `make_user`, `template_service.py`, `agenda_service.py`, `User`, `datetime.py`?**
...
- **Are the 88 inferred relationships involving `make_user()` (e.g. with `test_calendar_groups_executions_by_local_scheduled_date()` and `test_calendar_lists_group_executions_and_prioritizes_in_progress_status()`) actually correct?**
  _`make_user()` has 88 INFERRED edges - model-reasoned connections that need verification._
- **Are the 87 inferred relationships involving `make_group()` (e.g. with `test_calendar_groups_executions_by_local_scheduled_date()` and `test_calendar_lists_group_executions_and_prioritizes_in_progress_status()`) actually correct?**
  _`make_group()` has 87 INFERRED edges - model-reasoned connections that need verification._
...
- **Should `make_user` be split into smaller, more focused modules?**
  _Cohesion score 0.05305168104490849 - nodes in this community are weakly interconnected._